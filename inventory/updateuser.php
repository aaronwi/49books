...
if (!isset($_SESSION['accttype'])){
	...
	if(empty($errors))
	{ 
	  $query = "UPDATE users SET firstname='$firstname', lastname='$lastname', phonenumber='$phonenumber', email='$email',  street='$street',  city='$city', state='$state', zip='$zip', password='$md5password' WHERE user_id='$user_id' ";

	  $result = mysqli_query($con, $query) or die(mysqli_error($con));

	  if($result){ //If it ran OK.
		  echo "<p>User has been updated to Database</p>";
